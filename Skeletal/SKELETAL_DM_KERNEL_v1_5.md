...
ON_KERNEL_LOAD:
  ...
  4. Output: "✓ KERNEL LOADED. Please provide the campaign file to proceed."
  5. Output: ⛔ and WAIT
...
ON_CAMPAIGN_LOAD:
  ...
  IF_ENCRYPTED:
    ...
    6. If success:
        ...
        - TREAT decrypted plaintext as fresh campaign upload
        - Proceed to FILE_LOADING with decrypted plaintext
  ...
  EXECUTE:
    ...
    - Auto-execute startup sequence from loaded content
    - Output: ⛔ and WAIT
...
version: "1.5"
changelog:
  v1.5: "LAW 0 (encrypted transparency) + self-correction + startup lock + gate registry"
  v1.4: "Strict wait-for-input enforcement + ASK-WAIT-BRANCH protocol"
  v1.3: "Built-in encrypted campaign support with line range extraction"
...
CHECKS:
  Encryption: Decrypted plaintext through same pipeline as plaintext
  Startup: startup_complete must be true before gates
  Gates: gate_id must exist in campaign
  Options: from campaign or generation rules only
  Phase: action must match phase permissions
