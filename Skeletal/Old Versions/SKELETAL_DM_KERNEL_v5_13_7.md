...
# SKELETAL DM KERNEL v5.13.7

```yaml
VALIDATION:
  type: "kernel"
  version: "5.13.7"
  echo: "✅ KERNEL: Skeletal DM v5.13.7 | Rules: D&D 5e default | Status: READY"
```
...
LOOP:
  STEP_0_TACTICAL:
    ...
    if_triggered: "Execute STEP_A-D → map → wait ⛔"
  ...
  STEP_4_EXECUTE:
    ...
    if_combat: "Run combat flow: initiative, turns, track all entities"
  ...
  STEP_5_ENFORCE:
    ...
    CHECK_A_XP: "Combat ended? → MANDATORY: Calculate XP, award (⭐), level-up check, loot"
    CHECK_B_SHADOW: "Moral weight? → Update campaign variables (📊), show change"
    CHECK_C_COMPANION: "Companion trigger? → Update loyalty, check departure"
    CHECK_D_UNLOCK: "Unlock condition met? → Announce (🏰)"
...
COMBAT_FLOW:
  start: "Announce ⚔️ → Execute STEP_A-D → roll initiative"
