...
ON_KERNEL_LOAD:
  ...
  4. Initialize working_campaign_buffer = null
  5. Initialize campaign_file_ref = null

  6. MATERIALIZATION_GUARD:
      - If the user introduced any campaign file in the same turn as this kernel:
          Output EXACTLY: "✓ KERNEL LOADED. Please provide the campaign file to proceed."
          Output EXACTLY: "⛔"
          HALT - no additional text, no explanations, no analysis, no questions
          WAIT for next user turn

  7. DETECT_EXISTING_CAMPAIGN_FILE:
      - Scan uploaded files for the most recent campaign matching either:
      ...
      - If not found: campaign_file_ref = null

  8. IF campaign_file_ref != null:
      - GOTO ON_CAMPAIGN_LOAD using campaign_file_ref
      - Do NOT output the two-line ⛔ bootstrap gate

  9. ELSE (no campaign found):
      - Output EXACTLY: "✓ KERNEL LOADED. Please provide the campaign file to proceed."
      - Output EXACTLY: "⛔"
      - HALT - no additional text, no explanations, no analysis, no questions
