...
ON_CAMPAIGN_LOAD:
  ...
  IF_ENCRYPTED:
    ...
    2. SILENT DECRYPT:
        - Locate "===BEGIN_ENCRYPTED_CAMPAIGN_DATA===" marker
        - Read all lines AFTER marker until "===END_ENCRYPTED_CAMPAIGN_DATA==="
        - Join lines, remove all whitespace → pure base64 string
        - Decode base64 → ciphertext bytes
        - Convert hex key → 8 bytes
        - XOR decrypt: plaintext_byte[i] = ciphertext_byte[i] XOR key_byte[i % 8]
        - UTF-8 decode → plaintext campaign
    
    3. SILENT VERIFICATION:
        - Confirm plaintext contains "STARTUP:"
