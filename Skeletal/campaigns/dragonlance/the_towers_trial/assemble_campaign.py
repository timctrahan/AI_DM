...
def load_version():
    ...
    if not os.path.exists(VERSION_FILE):
        ...
        default_version = {
            ...
            "output_filename": "CAMPAIGN_The_Towers_Trial_v2.0.0.md",
            "campaign_name": CAMPAIGN_NAME,
            "part_files": PARTS,
            "versioning_rules": {
                "patch": "0-1 files changed",
                "minor": "2-3 files changed",
                "major": "4-6 files changed"
            }
        }
...
def calculate_new_version(current_version, changed_count):
    ...
    if changed_count <= 1:
        ...
        # 4-6 files: increment major, reset minor and patch
...
class CampaignAssembler:
    ...
    def assemble(self, version_string: str) -> str:
        ...
        # Build header
        assembled = self._build_header(version_string)
        ...
        # Add footer
        assembled += "\n\n" + self._build_footer(version_string)
...
def main():
    ...
    if changed_count <= 1:
        ...
        print(f"  Reason: 4-6 files changed (major increment)")

    # Generate output filename (unless overridden)
    if args.output is None:
        output_file = f"CAMPAIGN_The_Towers_Trial_{version_string}.md"
    else:
