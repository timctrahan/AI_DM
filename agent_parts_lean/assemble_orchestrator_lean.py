#!/usr/bin/env python3
"""
Assembly script for D&D 5E Orchestrator LEAN VERSION
Concatenates 6 modular parts into final orchestrator
Uses version.json for dynamic versioning based on file changes
Outputs to agent_parts_lean/ folder (does not overwrite main agent)
"""
...
def load_version():
    ...
    if not os.path.exists(VERSION_FILE):
        ...
        default_version = {
            ...
            "output_filename": "CORE_DND5E_AGENT_ORCHESTRATOR_LEAN_v1.0.0.md",
            "part_files": [
...
def update_part1_version(content, version_string):
    ...
    content = re.sub(
        ...
        f"# D&D 5E ORCHESTRATOR LEAN {version_string}",
        content
    )
...
def assemble_orchestrator(output_file=None):
    ...
    print("Loading version information (LEAN)...")
    ...
    # LEAN VERSION: Output to current directory (agent_parts_lean/)
    if output_file is None:
        output_file = f"CORE_DND5E_AGENT_ORCHESTRATOR_LEAN_{version_string}.md"
        output_path = output_file  # Stay in current directory
    else:
        output_path = output_file

    print(f"\nAssembling LEAN orchestrator...")
    ...
    # LEAN version should be smaller (expected ~37-42KB after removing context management)
    if size < 35000:
        print("  WARNING: File smaller than expected (expected ~37-42KB for LEAN)")
    elif size > 50000:
        print("  WARNING: File larger than expected (should be smaller than main version)")
    else:
        print("  Size looks good for LEAN version")

    # Backup part files to versioned archive
    try:
        backup_dir = ".previous_versions_lean"
    ...
    print(f"\nLEAN orchestrator created successfully!")
    print(f"Main orchestrator remains unchanged in parent directory.")
